...
from tfx_bsl.tfxio import tfxio as tfxio_base
  ...
  def _run_model_inference(
    ...
    with self._make_beam_pipeline() as pipeline:
      ...
      for split, example_uri in example_uris.items():
        ...
        data = (
            pipeline
            | 'RunInference[{}]'.format(split) >> _run_inference(
                tfxio, payload_format, inference_endpoint))
        ...
        if output_examples:
          ...
          _ = (
              data
              | 'WriteExamples[{}]'.format(split) >> _write_examples(
...
def _run_inference(
    pipeline: beam.Pipeline, tfxio: tfxio_base.TFXIO,
    payload_format: int,
    inference_endpoint: model_spec_pb2.InferenceSpecType
  ...
  return (
      pipeline
      | 'ReadData' >> tfxio.RawRecordBeamSource()
      | 'ParseExamples' >> beam.Map(_get_parse_fn(payload_format))
...
def _write_examples(prediction_log: beam.pvalue.PCollection,
                    output_example_spec: bulk_inferrer_pb2.OutputExampleSpec,
